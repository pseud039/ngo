...
export default function Marque() {
  useEffect(() => {
    const scrollText = gsap.utils.toArray(".support");
    const tl = horizontalLoop(scrollText, {
      repeat: -1,
    });
    let speedTween;
  ...
  return (
    <div className="scrolling-text overflow-hidden ">
      <div className="support flex whitespace-nowrap">
        {[...Array(3)].map((_, setIndex) =>
          marqueeItems.map((item, index) => (
            <div
              key={`${setIndex}-${index}`}
              className="boxes flex-shrink-0 px-8 py-3 text-black/60  text-xl uppercase font-bold"
            >
              {item}
            </div>
          ))
